...
echo -e "=== Beginning of SLiM run with different QTLs > $(date) ===" >>  ~/Mylab/evogen-sims/FluctPopulationModels.dir/Output.dir/NeutrSelInstatPop/NeutrSelInstatPop${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
    ...
    for SD in "${stdvs[@]}"; do
      for loci in "${!loci_to_regions[@]}"; do
        for gen in "${!generations_to_ranges[@]}"; do
          ...
          {
            slim -d seed=$seed -d repl=$repl -d loci=$loci -d region=$region -d h=$h -d gen=$gen -d rang=$rang -d SD=$SD NeutrSelInstatPop.slim
          } &
        done
      done
    done
...
echo "=== Finished! End of SLiM QTLs Constant Selection run > $(date). Total runtime: $total_runtime seconds ===" >> ~/Mylab/evogen-sims/FluctPopulationModels.dir/Output.dir/NeutrSelInstatPop/NeutrSelInstatPop${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
