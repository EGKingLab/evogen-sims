...
echo -e "=== Beginning of SLiM run with different QTLs > $(date) ===" >>  ~/Mylab/evogen-sims/FluctPopulationModels.dir/Output.dir/ConstSelInstatPop/ConstSelInstatPop${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
...
if [[ ! -f "$output" ]]; then
  ...
  seeds=($(awk -v seed=12345 'BEGIN { srand(seed); for (i=1; i<=100; i++) printf "%d ", int(4000 + rand() * (7000 - 4000)); }'))
    ...
    for SD in "${stdvs[@]}"; do
      for loci in "${!loci_to_regions[@]}"; do
        for gen in "${!generations_to_ranges[@]}"; do
          ...
          {
            slim -d seed=$seed -d repl=$repl -d loci=$loci -d region=$region -d h=$h -d gen=$gen -d rang=$rang -d SD=$SD ConstSelInstatPop.slim
          } &
        done
      done
    done
...
echo "=== Finished! End of SLiM QTLs Constant Selection run > $(date). Total runtime: $total_runtime seconds ===" >> ~/Mylab/evogen-sims/FluctPopulationModels.dir/Output.dir/ConstSelInstatPop/ConstSelInstatPop${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
