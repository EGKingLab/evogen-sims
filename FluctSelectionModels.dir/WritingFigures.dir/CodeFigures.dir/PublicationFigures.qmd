...
plot(seq(1,total_time), n_opt, type= "l")
```
```{r}
rm(list = ls())
seasons = c(12, 22, 10, 16);
...
pheno = rnorm(10000, 3,7);
mu_p  = mean(pheno);
sd_p = sd(pheno);

# season_id_1y = NULL;
# 
# for (i in 0:(length(seasons) - 1)) {
#     season_id_1y = c(season_id_1y, rep(i + 1, seasons[i]));
# }
# 
# t_within_1y = NULL;
# for(i in seasons){
#   t_within_1y = c(t_within_1y, seq(0,i-1)); 
#   }



# One-year indices
season_id_1y =rep(seq_along(seasons), times = seasons);  # 1,1,...,2,2,...,3,3,...,4,4,...
t_within_1y  <- sequence(seasons);                          # 1..L1, 1..L2, 1..L3, 1..L4

# Repeat to the desired length
season_id <- rep(season_id_1y, length.out = total_time);


# alten_id = NULL
# for (sea_id in season_id_1y) {
#
# }
n_opt = NULL
t_within  <- rep(t_within_1y,  length.out = total_time);

L_t   <- seasons[season_id];
amp_t <- sd_values[season_id] * sd_p;

n_opt <- mu_p + amp_t * sin(2 * pi * t_within / L_t);

plot(seq_len(total_time), n_opt, type = "l", xlab = "Generation", ylab = "Optimum")
```
...
```{r, warning=FALSE, fig.width = 30, fig.height = 40, dpi = 600, echo=FALSE}
...
```{r, warning=FALSE, fig.width = 23, fig.height = 15, echo=FALSE, dpi = 1200}
#| label: Instant_Poly_SpecAnalysis
library(tidyverse)
