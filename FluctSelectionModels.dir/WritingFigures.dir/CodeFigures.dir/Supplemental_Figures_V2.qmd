...
packages <- c("dplyr", "stringr", "plotly", "patchwork", "cowplot", "doParallel", "ggplot2", "foreach", "future", "parallel")
...
process_files <- function(dirpath, pattern) {
  ...
  # My list for dataframes
  dataframes <- list()
  
  # Get parameters from the file and keep dataset based on file name
  
  for (file in files) {
    fname <- basename(file) # file name
    replicate <- as.numeric(str_extract(fname, "(?<=genome)\\d+")) # repl number 
    H         <- as.numeric(str_extract(fname, "(?<=H)0\\.\\d+")) # h2 value
    loci      <- as.numeric(str_extract(fname, "(?<=_n)\\d+")) # Number of loci
    SD        <- as.numeric(str_extract(fname, "(?<=SD)\\d+")) # Distance to optimum
    Gen       <- as.numeric(str_extract(fname, "(?<=Gen)\\d+")) # Selection length
...
process_files <- function(dirpath, pattern) {
  ...
  for (file in files) {
    fname <- basename(file)  # file name
    replicate <- as.numeric(str_extract(fname, "(?<=genome)\\d+"))  # replicate number 
    H         <- as.numeric(str_extract(fname, "(?<=H)0\\.\\d+"))      # h2 value
    loci      <- as.numeric(str_extract(fname, "(?<=_n)\\d+"))         # Number of loci
    SD        <- as.numeric(str_extract(fname, "(?<=SD)\\d+"))          # Distance to optimum
    Gen       <- as.numeric(str_extract(fname, "(?<=Gen)\\d+"))         # Selection length
...
## 4.1. Neutral selection Alle frequency
...
# #| fig-cap: "SI 4.1. Allele frequency dynamics for a single replicate from all genetic architectures where each line in represents the frequency trajectory each locus."
# #| fig-pos: "H"
# #| 
# dirpath <- "../../Output.dir/NS.dir/"
...
# #| fig-cap: "SI 5.1.1. Monogenic single replicate allele frequency plot for constant selection, showing parameter interactions."
# #| fig-pos: "H"
# 
