...
echo -e "=== Beginning of SLiM run with different QTLs > $(date) ===">>../../Output.dir/SinFS.dir/FS_OptSin${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log

output="/storage/hpc/group/kinglab/etb68/evogen-sims/ET_Yeast/Output.dir/SinFS.dir/genome5_100_0.5.csv"
if [[ ! -f "$output" ]]
then
...
for seed in "${!seeds_to_replicates[@]}"; do
  ...
  do
    ...
    do
	  ...
	  {
            slim -d seed=$seed -d repl=$repl -d loci=$loci -d region=$region -d h=$h -d gen=$gen -d rang=$rang -d SD=$SD GradI_Mod.slim
          } &
        done
      done
    done
...
echo "=== Finished! End of SLiM QTLs fluctuating 2 sinusoidal run > $(date). Total runtime: $total_runtime seconds ===" >>../../Output.dir/SinFS.dir/FS_OptSin${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
