...
echo -e "=== Beginning of SLiM run with different QTLs > $(date) ===" >> ../../Output.dir/LinFS.dir/FS_par4${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
if [[ ! -f "$output" ]]
then
    ...
    for SD in "${stdvs[@]}"; do
      for loci in "${!loci_to_regions[@]}"; do
        for gen in "${!generations_to_ranges[@]}"; do
          ...
          {
            slim -d seed=$seed -d repl=$repl -d loci=$loci -d region=$region -d h=$h -d gen=$gen -d rang=$rang -d SD=$SD FS_Mod.slim
          } &
        done
      done
    done
...
echo "=== Finished! End of SLiM QTLs fluctuating 2 equal seasons run > $(date). Total runtime: $total_runtime seconds ===" >> ../../Output.dir/LinFS.dir/FS_par4${SLURM_JOBID}_${SLURM_ARRAY_TASK_ID}.log
