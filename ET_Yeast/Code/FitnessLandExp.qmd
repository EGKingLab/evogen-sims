...
newdataset <- df %>% group_by(sd, locus, h2, optimum) %>%
  mutate(
    ...
    stand_dev = factor(sd),
    Fitness = ifelse(locus == 1, exp(-(1/150) * (phenotype - optimum)^2),
                     exp(-(1/(150 * sqrt(locus))) * (phenotype - optimum)^2))
  )
...
#   #dplyr::filter(h2 > 0.1) %>% 
#   dplyr::select(optimum, sd, heritability, locus, Fitness, phenotype) %>% 
#   distinct() %>% 
...
# theme_set(theme_classic())
# 
# ggplot(newdataset, aes(x = phenotype, y = Fitness, fill = heritability)) +
#   #geom_density_ridges(aes(fill = heritability), alpha = 0.3) + 
#   geom_histogram()+
#   #scale_fill_manual(values = c("cyan", "yellow", "magenta")) +
#   facet_grid(~locus, scales = "free_y")+
...
```{r}
newdataset2 <- df %>% group_by(seed, sd, locus, h2) %>%
  mutate(heritability = factor(h2),
    stand_dev = factor(sd),
    Fitness = as.factor(round(Fitness, digits = 5)))
    ...
    for (h2 in h2s) {
      ...
      for (stand in sds) {
        myphedata <- tibble(
          phenotypes = additive + env,
          mup = 0.5 * locus, # Expected mean
          sdp = ifelse(locus == 1, sqrt((3*locus) / h2), sqrt(locus / (3 * h2))), 
          phenotype = (phenotypes - mup) / sdp, # Phenotype scaling
          optimum = mean(phenotype) + (stand * sd(phenotype)),
          Fitness = ifelse(locus <= 10, exp(-(1/100) * (phenotype - optimum)^2), 
                           exp(-(1/(100*sdp)) * (phenotype - optimum)^2)),
          stand = stand,  
          h2 = h2,  
          locus = locus
        )
...
newdataset <- combined_df %>%
  ...
  mutate(phenotype = (phenotypes - mup) / sdp, # Scaled phenotype
         ...
         Fitness = case_when(
           locus == 1 ~ exp(-(sd(phenotype)/4) *  (phenotype - optimum)^2),
...
```{r, fig.width=12, fig.height=6}
theme_set(theme_classic())
ggplot(newdataset %>% filter(standv == 4, heritability == 0.5, locus <= 100) %>%
