...
sds <- c(0, 1, 4, 6, 8) # Standard deviations for optimum

# dataframe
df_list <- list()  # List to store iterations
for (iteration in 1:2000) { # 1000 iterations
...
newdataset <- combined_df %>%
  ...
  mutate(
    #phenotype = 10 + (phenotypes - min(phenotypes)) / (max(phenotypes)* min(phenotypes)), 
         phenotype = (phenotypes - mup) / sdp,# Scaled phenotype
         optimum = (standv), # mean(phenotype) +  * sd(phenotype)), # Optimum phenotype
         #Fitness = case_when(
...
newdataset2 <- newdataset %>% 
  filter(heritability == 0.1,locus <= 100, standv == 4) # | standv == 4| standv == 8
# Define the phenotype value
pheno.value <- 0#1.60

# Filter the dataset to get fitness values where phenotype is 1.6 for each locus and standv
fit.values <- newdataset2 %>%
  ungroup() %>%
  filter(round(phenotype, digits = 2) == 0) %>%
