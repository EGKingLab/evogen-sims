...
```{r}
mm.all |> filter(h2F == "0.1", nlociF==300) %>% 
  ggplot(aes(value, color = h2F, group = rep_code)) +
  geom_density(size = 0.1) + 
...
# Forget about this section. It's to explore
...
for(kk in 1:nrow(pp))
{
  ...
  for (ii in 1:niter) {
    ...
    for (ind in 1:ninds) {
      geno = 0
      for(ii in seq_len(nloci)){
      geno[ii] <- rbinom(1, 1, markers)
      }
      effs <- geno*effects
      additive[ind] <- sum(effs)
    }
...
```{r}
mm.all |> filter(h2F == "0.8", nlociF==300) %>% 
  ggplot(aes(value, color = h2F, group = rep_code)) +
  geom_density(size = 0.1) + 
  facet_grid(nlociF~., scales = "free_x")
```