...
#         facet_wrap(~h2_sd, ncol = 4, labeller = label_both) +
#         ylim(min = 0, max = 1) +
#         theme_bw() +
...
```{r}
dirpath <- "/mnt/pixstor/kingeg-lab/etb68/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/NS.dir/"
pattern <-  "^genome15\\D" # "^genome(1|2|3)_n\\d+_H0.1_*" # "^genome15\\D" #(\\D|$)
...
```{r}
dirpath <- "/mnt/pixstor/kingeg-lab/etb68/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/CS.dir/"
pattern <- "^genome1\\D" #"^genome(1|2|3)_n\\d+_H*" #"^genome1\\D" #(\\D|$)
result <- process_files(dirpath, pattern, "loci_gen")
combined_data <- result$combined_data
plots <- result$plots
plots
...
pattern <- "^genome15\\D"#_n\\d+_H0.1_*" #(\\D|$)
result <- process_files(dirpath, pattern, "loci_gen")
combined_data <- result$combined_data
plots <- result$plots
plots
