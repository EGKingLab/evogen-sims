...
process_files <- function(dirpath, pattern, plot_type) {
  ...
  if (plot_type == "loci_gen") {
    ...
    for (loci_geni in loci_gens) {
      ...
      unique_repl <- unique(locus_data$repl)
      if (length(unique_repl) > 30) {
        selected_repl <- sample(unique_repl, 30)
        locus_data <- locus_data %>%
          filter(repl %in% selected_repl)
      }
      
      p <- ggplot(locus_data, aes(Generation, Phenotype, color = factor(repl))) +
        geom_line(alpha = 0.5) +
...
process_files <- function(dirpath, pattern, plot_type) {
  ...
  # Create plot
  p <- ggplot(summary_stats, aes(x = Generation, y = mean_phenotype, color = factor(repl), group = repl)) +
    geom_line() +
