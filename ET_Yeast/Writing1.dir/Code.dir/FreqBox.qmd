...
## Instantaneous all replicates 10 loci, 0.5, sd4, gen 30
...
pattern <- "^genome\\d+_n10_H0.5SD4Gen30"
...
## Instantaneous single replicate 30 out of 300 loci, 0.5, sd3, gen 30
...
## Instantaneous single replicate 30 out of 70 loci, 0.8, sd3, gen 30
...
## Instantaneous single replicate all parameters for at most 30 out of loci, h2 = 0.1
...
## SinI single replicate all parameters for at most 30 out of loci
...
## SinII Single replicate for all
...
# List of required packages
packages <- c("stringr", "plotly", "patchwork", "cowplot", "purrr", "doParallel")
...
for(loci_geni in loci_gens){
  ...
  p <- ggplot(locus_data, aes(x = Generation, y = factor(Position), fill = Frequency)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
          legend.position = "bottom",
          plot.title = element_text(hjust = 0.5)) +
    facet_wrap(~h2_sd, scales = "free_x", ncol = 4)

  plots[[loci_geni]] <- p
}
```

```{r, fig.height=6, fig.align='center', fig.width=12, echo=FALSE}
plots
```
...
heatmap_plot <- heatmap_plot +
  geom_text(data = filtered_dataset %>% filter(!is.na(Effect)), 
            aes(x = max(Generation) + 50, y = factor(Position), label = round(Effect, 1)), 
