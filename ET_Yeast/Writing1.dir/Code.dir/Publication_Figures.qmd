...
plot_fitness <- function(C) {
  ...
  p <- ggplot() +
    ...
    guides(color = guide_legend(ncol = 3, title = 'Distance to Optimum', size = 20)) #+  # Increased size for legend guide
...
> The one for CS can reflect what Abdul-Rahman et 2022 discusess in their paper
...
# Function to read in the files based on the specified parameters and extract Selection, H, SD, and Gen values using regex
read_files <- function() {
  file_list <- list.files(path = "~/YeastProj.dir/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/CS.dir/", pattern = "genome10_n(1|10|70)_H0\\.(1|8|5)(SD(1|2|4)).csv$", full.names = TRUE)
...
freqplot <- filtered_dataset %>%
  ...
  scale_color_identity() +
  scale_linewidth_identity() +
  facet_grid(Selection ~ H + SD, labeller = labeller(
    Selection = function(x) x,
    H = label_both,
    SD = label_both
  )) +
...
# Function to read in the files based on the specified parameters and extract Selection, H, SD, and Gen values using regex
read_files <- function() {
  file_list <- list.files(path = "~/YeastProj.dir/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/NS.dir/", pattern = "genome10_n(1|10|70)_H0\\.(1|8|5)\\.csv$", full.names = TRUE)
  
  datasets <- lapply(file_list, function(file) {
    data <- read.csv(file)
    matches <- str_match(file, "genome10_n([0-9]+)_H0\\.([0-9\\.]+)")
    selection_type <- as.numeric(matches[2])
    selection_label <- ifelse(selection_type == 1, "Monogenic",
                              ifelse(selection_type == 10, "Oligogenic", "Polygenic"))
...
freqplot <- filtered_dataset %>%
  ...
  scale_color_identity() +
  scale_linewidth_identity() +
  facet_grid(Selection ~ H, labeller = labeller(
    Selection = function(x) x,
    H = label_both
  )) +
  theme_cowplot() +
...
heatmap_plot <- heatmap_plot +
  ...
  filter(Generation <= 2000)  %>% filter(!is.na(Effect)), 
            ...
            hjust = 0, vjust = 0.15, size = 10, angle = 30, fontface = "bold")
...
heatmap_plot <- heatmap_plot +
  ...
  filter(Generation <= 2000)  %>% filter(!is.na(Effect)), 
            aes(x = max(Generation) + 15, y = factor(Position), label = round(Effect, 1)), 
            hjust = 0, vjust = 0.15, size = 10,  fontface = "bold", angle = 30)
...
heatmap_plot <- heatmap_plot +
  ...
  filter(Selection=="Polygenic")  %>% filter(!is.na(Effect)),
            aes(x = max(Generation) + 15, y = factor(Position), label = round(Effect, 1)),
            hjust = 0, vjust = 0.15, size = 10, fontface = 10, angle = 30)
