...
Is there any better way to present this? all i want is to see the effect of qtl number, size effect, and initial allele frequency. this should al be different across heritability values, standard deviations, and generations.
...
pattern <- "^genome\\d+_n(1|10|70|300)_H0.(1|5|8)(SD[1|4])?Gen(10|30)?\\.csv$"
...
plot <- ggplot(data = newd, aes(x = as.factor(loci), y = Perc_Loci, color = as.factor(loci))) +
  ...
  geom_text(aes(label = round(Perc_Loci, 2)), vjust = 2.5, hjust = 0.5, size = 10, fontface = "bold") +
  facet_grid(
    Fixation + Gen  ~ H +  SD,
    labeller = label_both
  ) +
  ...
  guides(color = guide_legend(nrow = 1, byrow = TRUE))  # Arrange legend items in a single row
...
```{r}
newdd <- newd %>% ungroup() %>% mutate(Param_Combo = paste0(H_label," ", SD_label ," ", Gen_label)) %>% 
  dplyr::select(loci, Fixation, Perc_Loci, Param_Combo)

newdd_wide <- newdd %>%
  pivot_wider(names_from = Param_Combo, values_from = Perc_Loci)
...
#### Dot Plots for all Fixattion status
...
path <- "~/YeastProj.dir/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/LinFS.dir/"
pattern <- "^genome\\d+_n(1|10|70|300)_H0.(1|5|8)(SD[1|4])?(Gen10|30)?\\.csv$"
...
# Assuming mydata4 is your dataframe
# Separate the Loci_h_gen_sd column into four new columns
 library(tidyr)

# Separate the "Loci_h_gen_sd" column into four columns
mydata5 <- mydata4 %>%
  separate(Loci_h_gen_sd, into = c("Loci", "Herit", "Gen", "sd"), sep = " ", remove = FALSE) %>%
  mutate(
    Loci = as.numeric(sub("Loci = ", "", Loci)),
    Herit = as.numeric(sub("Herit = ", "", Herit)),
    Gen = as.numeric(sub("Gen = ", "", Gen)),
    sd = as.numeric(sub("sd = ", "", sd))
  )

# Create a summary data frame
summary_df <- mydata5 %>%
  group_by(Loci, h, gen, sd) %>%
  summarize(
    num_fixed = sum(Fixation == "Fixed", na.rm = TRUE),
    num_lost = sum(Fixation == "Lost", na.rm = TRUE)
  ) %>%
...
# Apply the custom labels to the plot
ggplot(mydata5, aes(x = Effect, y = InitFreq, size = Effect, color = Fixation)) +
  geom_point(alpha = 0.5) +
  geom_text(aes(label = paste0(sprintf("%.2f", Effect))), 
            vjust = 1, hjust = 1, size = 5, check_overlap = TRUE, fontface = "bold") +
  labs(title = "Relationship among Position, Fixation, Effect, and Initial Frequency",
       ...
       color = "Fixation Status",
       size = "Effect") +
  facet_wrap(Loci + h + sd ~ gen, ncol = 4, scales = "free_y") +  # Use actual facet values
  geom_label(data = summary_df, aes(x = Inf, y = Inf, label = label), 
            vjust = 1.1, hjust = 1.1, size = 10, fontface = "bold", inherit.aes = FALSE, label.padding = unit(0.2, "lines"), label.size = 0.5) +
  theme_classic() +
  theme(
...
## Now we can compare Fixed and Lost Numbers from Gradual vs Instant
...
pattern <- "^genome\\d+_n(300)_H0.(1|8)(SD[1|4])?(Gen(10|30))?\\.csv$"
...
mydata3 <- mydata2 %>% dplyr::select(Generation, Position, Fixation, Frequency, Effect, InitFreq, gen, repl, herit, sd,loci, Loci_h_gen_sd)
```
...
ggplot(mydata5, aes(x = Effect, y = InitFreq, size = Effect, color = Fixation)) +
  ...
  labs(title = "Relationship among Position, Fixation, Effect, and Initial Frequency",
       ...
       color = "Fixation Status",
       size = "Effect") +
  facet_grid(loci + herit + sd ~ gen, scales = "free_y") +  # Use actual facet values
  geom_label(data = summary_df, aes(x = Inf, y = Inf, label = label), 
            vjust = 1.1, hjust = 1.1, size = 10, fontface = "bold", inherit.aes = FALSE, label.padding = unit(0.2, "lines"), label.size = 0.5) +
  theme_cowplot() +
  theme(
...
ggplot(mydata5, aes(x = Effect, y = InitFreq, size = Effect, color = Fixation)) +
  geom_point(alpha = 0.5) +
  geom_text(aes(label = paste0(sprintf("%.2f", Effect))), 
            vjust = 1, color = "black", hjust = 1, size = 5, check_overlap = TRUE, fontface = "bold") +
  labs(title = "Relationship among Position, Fixation, Effect, and Initial Frequency",
       x = "Effect",
       y = "Initial Frequency",
       color = "Fixation Status",
...
mydata3 <- mydata2 %>% dplyr::select(Generation, Position, Fixation, Frequency, Effect, InitFreq, repl, herit, sd,loci, Loci_h_gen_sd)
```
...
ggplot(mydata5, aes(x = Effect, y = InitFreq, size = Effect, color = Fixation)) +
  ...
  labs(title = "Relationship among Position, Fixation, Effect, and Initial Frequency",
       ...
       color = "Fixation Status",
       size = "Effect") +
  facet_grid(herit + sd ~ loci, scales = "free_y") +  # Use actual facet values
  geom_label(data = summary_df, aes(x = Inf, y = Inf, label = label), 
            vjust = 1.1, hjust = 1.1, size = 10, fontface = "bold", inherit.aes = FALSE, label.padding = unit(0.2, "lines"), label.size = 0.5) +
  theme_cowplot() +
  theme(
