...
process_plots <- function(file) {
 ...
 p <- ggplot(data, 
         aes(Generation, Frequency, group = interaction(postion_effect_init, replicate), color = interaction(postion_effect_init, replicate)))+
    geom_line(size = 0.5)+
    labs(x = "", y = "") +
    ylim(min = 0, max = 1)+
    theme_bw()#+
...
path <- "~/YeastProj.dir/evogen-sims/ET_Yeast/Writing1.dir/Output.dir/Add_Check.dir/LinFS.dir/"
...
> In this section, we can explore the sequential difference between of allele frequency, set a threshold, then select loci with high fluctuations. After we sample 10 or more of those loci for more exploration

```{r, fig.width = 16, fig.height = 8}

dirpath <- "~/YeastProj.dir/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/LinFS.dir/"
pattern <- "^genome1_n70_.*\\.csv$"#(15|2|3)
files <- list.files(dirpath, pattern, full.names = TRUE)
...
# Ensure gen is not greater than the size of your data
#gen <- min(gen, nrow(data) - 1)

datadiff <- data %>%
  select(-Effect) %>%
  group_by(Position) %>%
  pivot_wider(names_from = Position, values_from = Frequency) %>%
  mutate_at(vars(-c(Generation)), ~c(rep(0, gen), diff(., lag = gen))) %>%
#  mutate_at(vars(-c(Generation)), ~c(rep(0, gen), diff(.))) %>%
...
afm <- datadiff  %>% filter(mu_fr < mean(mu_fr) - sd(mu_fr)/sqrt(length(mu_fr))) %>% # /sqrt(length(mu_fr))
  mutate(Position = factor(Position))
...
p <- ggplot(locus_data, aes(Generation, Frequency, group = position_effect_init, color = position_effect_init))+
    geom_line(size = 0.5)+ #interaction(position_effect_init), replicate
    facet_wrap(~h2_sd, ncol = 4)+
    ylim(min = 0, max = 1)+
    theme_bw()
  #theme(legend.position = "none")
...
# dirpath <- "~/YeastProj.dir/evogen-sims/ET_Yeast/output.dir/Selection_Models/WF.dir/LinFS.dir/"
# pattern <- "^genome1_n300_.*\\.csv$"#(15|2|3)
# files <- list.files(dirpath, pattern, full.names = TRUE)
# 
# process_file <- function(file) {
#   replicate_id <- as.numeric(str_extract(file, "(?<=genome)\\d+"))
...
#     summarise(mu_fr = mean(abs(Frequency), na.rm = TRUE)) %>%
#     arrange(mu_fr) %>% replace_na(list(mu_fr = 0))
#   afm <- datadiff  %>% filter(mu_fr > mean(mu_fr) - sd(mu_fr)/sqrt(length(mu_fr)))%>% # median(mu_fr))
#     mutate(Position = factor(Position))
# 
#   filtered_positions <- (afm$Position)
# 
...
#   #locus_data <- locus_data %>% filter(Position %in% selected_positions)
# 
#   p <- ggplot(locus_data, aes(Generation, Frequency, group = interaction(position_effect_init, replicate), color = interaction(position_effect_init, replicate)))+
...
```{r}
path <- "~/YeastProj.dir/evogen-sims/ET_Yeast/Writing1.dir/Output.dir/Add_Check.dir/SinFSGen.dir/"
...
process_file <- function(file) {
  ...
  datadiff <- data %>%
    ...
    pivot_wider(names_from = Position, values_from = Frequency) %>%
    mutate_at(vars(-c(Generation)), ~c(rep(0, gen), diff(., lag = gen)))  %>%
    # Uncomment for sequential difference
