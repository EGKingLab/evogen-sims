...
mythemes <- theme_bw() +
  theme(
    ...
    axis.text.x     = element_text(face = "bold", size = 30, angle =25, margin = margin(t = 20), hjust = 1),
...
```{r, echo=FALSE,fig.width=10, fig.height=5, dpi = 1200, warning=FALSE}
#library(tidyverse)
...
p <- ggplot(df, aes(x = optimum, y = fitness_at_zero)) +
  ...
  geom_text(data = df_label, 
            ...
            vjust = -0.8, hjust = -1, size = 8, angle= 270,color = "darkred") +
  ylim(0.85, 1)+
  labs(
...
```{r, warning=FALSE, fig.width = 50, fig.height = 60, dpi = 300, echo=FALSE, message=FALSE}
...
# library(doParallel)
# library(stringr)
# library(readr)  # For read.csv function
# 
