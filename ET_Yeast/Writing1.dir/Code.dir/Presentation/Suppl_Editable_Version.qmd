...
> For the allele frequencies, I will in some cases use ribbons to show changes across replicates. Then I will use a single replicate to track each locus.

## 1. Instantaneous polygenic model
...
# Loop over parameter combinations and create violin plots
...
library(cowplot)
labels <- LETTERS[1:3]
plot_grid(plotlist = c(plots[7], plots[7], plots[9]), ncol = 1, labels = labels, label_size = 60, label_colour = "red")
```
...
run_fft_analysis <- function(folder_path, pattern, spectrum_span = 2) {
  ...
  files_df <- data.frame(file_name = file_list) %>%
    mutate(
      ...
      replct = str_extract(base_name, "genome\\d+"), # Allow varying genome numbers
...
library(cowplot)
labels <- LETTERS[1:3]
plot_grid(plotlist = c(result[7], result[7], result[9]), ncol = 1, labels = labels, label_size = 60, label_colour = "red")
```
