...
Opti_Gen <- tibble(Generation = seq(1:200),
                   Optimum1 = rep(c(rep(4, 25), rep(-4, 25)), 4))
...
p2 <- ggplot()+
  ...
  annotate("text", x = 82, y = mu_pheno + 0.1, 
           label = "Mean Phenotype", 
           size = 5, vjust = 0.1, hjust = 0., 
           col = "darkred", angle = 85)+
  #labs(x = NULL, y = NULL)+
...
p4 <- ggplot(df %>% filter(Generation < 502)) + 
    geom_line(aes(Generation, Optimum), size = 2) +
    geom_hline(yintercept = mean(Optimum), color = "darkred", size = 2) +
    annotate("text", x = 70, y = 0.2, label = "Mean Phenotype", size = 5, vjust = 0.1, hjust = 0, col = "darkred", angle = 80) + 
    #labs(x = NULL, y = NULL) +
  labs(x = "Generation", y = "Optimum")+
    theme_cowplot() +
    theme(legend.position = "none",
